...
const REACT_AUTH_STORAGE_KEY = 'gs2cb.user';
const REACT_TOKEN_STORAGE_KEY = 'gs2cb.token';
...
function normalizeDiscordAuth(user) {
  ...
  const token = user.token || readCookie(REACT_TOKEN_STORAGE_KEY) || '';
  const username = user.username || user.discordId || user.discord_id || '';
  const avatarUrl = user.avatarUrl || user.avatar_url || '';
  const discordId = user.discordId || user.discord_id || (String(username).startsWith('discord:') ? String(username).slice(8) : '');
  return { ...user, token, username, avatarUrl, discordId, roles, botAdmin, botEditor };
}
...
function readDiscordAuth() {
  try {
    ...
    const auth = raw ? normalizeDiscordAuth(JSON.parse(raw)) : readReactAuth();
    if (auth) writeDiscordAuth(auth);
    return auth;
  } catch {
    return readReactAuth();
  }
}

function readCookie(name) {
  return document.cookie.split('; ').find(row => row.startsWith(name + '='))?.split('=').slice(1).join('=') ? decodeURIComponent(document.cookie.split('; ').find(row => row.startsWith(name + '='))?.split('=').slice(1).join('=') || '') : '';
}

function readReactAuth() {
  try {
    const raw = localStorage.getItem(REACT_AUTH_STORAGE_KEY);
    const token = localStorage.getItem(REACT_TOKEN_STORAGE_KEY) || readCookie(REACT_TOKEN_STORAGE_KEY);
    if (!raw || !token) return null;
    const user = JSON.parse(raw);
    return normalizeDiscordAuth({
      token,
      ...
      role: user.role || 'user',
      roles: user.roles || '',
      botAdmin: user.role === 'admin' || user.canManageShowcase,
      botEditor: user.role === 'editor',
      canManageShowcase: user.canManageShowcase,
      canPostShowcase: user.canPostShowcase,
      isShowcaseBlocked: user.isShowcaseBlocked
    });
  } catch {
    return null;
  }
...
function writeDiscordAuth(user) {
  const auth = normalizeDiscordAuth(user);
  if (!auth) return;
  localStorage.setItem(DISCORD_AUTH_STORAGE_KEY, JSON.stringify(auth));
  if (auth.token) localStorage.setItem(REACT_TOKEN_STORAGE_KEY, auth.token);
  localStorage.setItem(REACT_AUTH_STORAGE_KEY, JSON.stringify({
    username: auth.username || '',
    nickname: auth.nickname || null,
    avatarUrl: auth.avatarUrl || null,
    role: auth.botAdmin ? 'admin' : (auth.botEditor ? 'editor' : 'user'),
    canManageShowcase: !!auth.botAdmin,
    canPostShowcase: auth.canPostShowcase !== false,
    isShowcaseBlocked: !!auth.isShowcaseBlocked
  }));
}

function clearDiscordAuth() {
  localStorage.removeItem(DISCORD_AUTH_STORAGE_KEY);
  localStorage.removeItem(REACT_AUTH_STORAGE_KEY);
  localStorage.removeItem(REACT_TOKEN_STORAGE_KEY);
  document.cookie = REACT_TOKEN_STORAGE_KEY + '=; path=/; max-age=0; samesite=lax';
  document.cookie = REACT_TOKEN_STORAGE_KEY + '=; path=/; max-age=0; samesite=lax; domain=.gscript.dev; secure';
}
...
function DocsMonacoExampleEditor({ name, defaultValue = '', placeholder = '', ariaLabel = 'Example' }) {
  ...
  React.useEffect(() => {
    ...
    ensureDocsMonaco().then(monacoInstance => {
      ...
      if (hiddenRef.current) hiddenRef.current.__docsMonacoEditor = editor;
      setReady(true);
    ...
    return () => {
      ...
      if (editorRef.current) {
        if (hiddenRef.current) delete hiddenRef.current.__docsMonacoEditor;
...
function GSDoc() {
  ...
  const readDefinitionForm = React.useCallback(function(form, fallback, fallbackKey) {
    ...
    const read = function(field) {
      const el = form && form.querySelector('[name="' + field + '"]');
      if (el && el.__docsMonacoEditor) return el.__docsMonacoEditor.getValue();
      return (el && el.value) || fallback[field] || '';
    };
